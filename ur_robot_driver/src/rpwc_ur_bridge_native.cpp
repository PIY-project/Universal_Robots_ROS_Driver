...
void thread_pub_joint_states()
{
    ...
    {
        ROS_FATAL_STREAM("[joint_states]: Parameter '" << name_space_
                                                       << "/ur_hardware_interface/joints' not found on param server");
            ...
            Eigen::Vector3d &pos, Eigen::Quaterniond &quat, Eigen::Quaterniond &quat_old)
{
    ...
    double sign_check =
        quat.w() * quat_old.w() + quat.x() * quat_old.x() + quat.y() * quat_old.y() + quat.z() * quat_old.z();
    if (sign_check < 0.0)
    {
        quat.w() = quat.w() * (-1);
        quat.vec() = quat.vec() * (-1);
            ...
            std::vector<float> blending_radiuses)
{
    ...
    {
        rot = KDL::Rotation::Quaternion(waypoints[i].orientation.x, waypoints[i].orientation.y, waypoints[i].orientation.z,
        ...
        targets.push_back(std::make_shared<urcl::control::MoveLPrimitive>(pose, blending_radiuses[i],
                                                                          std::chrono::milliseconds(0), acc, vel));
    }
            ...
            std::vector<float> blending_radiuses)
{
    ...
    for (long unsigned int i = 0; i < waypoints.size(); i++)
    {
        ...
        targets.push_back(std::make_shared<urcl::control::MoveJPrimitive>(joints, blending_radiuses[i],
                                                                          std::chrono::milliseconds(0), acc, vel));
    }
...
{
    if (!execution_done.load() && work_thread && work_thread->joinable())
    {
        ROS_WARN("[Cartesian Move]: Another goal is already running, interrupting!");
        preempt_callback();
    }
    ...
    execution_done.store(false);
    if (work_thread && work_thread->joinable())
        work_thread->join();

    work_thread = std::make_unique<std::thread>(std::thread(&CartesianMove::execution_thread, this));
}

void CartesianMove::execution_thread()
{
    ROS_INFO("[Cartesian Move Execution]: Executing trajectory");
    ...
    ROS_INFO("[Cartesian Move Execution]: Execution done");

    if (preempted.load())
    {
        ROS_INFO("[Cartesian Move Execution]: Goal already preempted");
        preempted.store(false);
        return;
    }

    if (!rpwc_result.success)
    {
        ROS_INFO("[Cartesian Move Execution]: Goal aborted");
        rpwc_result.msg = "Goal failed";
        as.setAborted(rpwc_result, rpwc_result.msg);
        return;
    }

    ROS_INFO("[Cartesian Move Execution]: Goal completed");
...
void JointsMove::goal_callback()
{
    if (!execution_done.load() && work_thread && work_thread->joinable())
    {
        ROS_WARN("[Joints Move]: Another goal is already running, interrupting!");
        preempt_callback();
    }
    ...
    if (len == 0)
    {
        ...
        rpwc_result.success = false;
        rpwc_result.msg = "No data found";
        rpwc_result.error_code = rpwc::errorArmMotionCode::GENERIC_ERROR;
        as.setAborted(rpwc_result, rpwc_result.msg);
        return;
    }

    active_goal_waypoints.clear();
    active_goal_waypoints.reserve(rpwc_goal->targets.size());
    ...
    for (auto it = rpwc_goal->targets.begin(); it != rpwc_goal->targets.end(); it++)
    {
        ...
        active_goal_waypoints.push_back(tmpWaypoint);
    }
    ...
    work_thread = std::make_unique<std::thread>(std::thread(&JointsMove::execution_thread, this));
}

void JointsMove::execution_thread()
{
    ROS_INFO("[Joints Move Execution]: Executing trajectory");
    rpwc_result.success = move_j(active_goal_waypoints, rpwc_goal->velocities, rpwc_goal->accelerations, rpwc_goal->zone_radiuses);

    if (preempted.load())
    {
        ROS_INFO("[Joints Move Execution]: Goal already preempted");
        preempted.store(false);
        return;
    }
    ...
    if (!rpwc_result.success)
    {
        ROS_INFO("[Joints Move Execution]: Goal aborted");
        rpwc_result.msg = "Goal failed";
        rpwc_result.error_code = rpwc::errorArmMotionCode::NATIVE_PATH_ERROR;
        as.setAborted(rpwc_result, rpwc_result.msg);
        return;
    }

    ROS_INFO("[Joints Move Execution]: Goal completed");
    rpwc_result.msg = "Goal succeded";
    rpwc_result.error_code = rpwc::errorArmMotionCode::NO_ERROR;
    as.setSucceeded(rpwc_result, rpwc_result.msg);
}

void JointsMove::preempt_callback()
{
    ROS_INFO("[Joints Move]: Goal preempt requested");
    preempted.store(true);
    ur_instruction_executor_->cancelMotion();
